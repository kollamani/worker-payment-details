...
export const BucketTaskCard = ({ note, onEdit, onView, onDelete, onToggleComplete }) => {
  ...
  return (
    <article className="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-200 hover:shadow-md">
      ...
      <p className="mt-2 font-mono text-lg font-bold tabular-nums text-slate-900">{formatINR(amountOfNote(note))}</p>

      {note.note?.trim() ? (
        <p className="mt-1.5 line-clamp-2 text-[11px] italic leading-4 text-slate-500" title={note.note}>
          {note.note}
        </p>
      ) : null}

      <dl className="mt-3 divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-100">
        {rows.map((row) => (
          <div key={row.label} className="flex items-center justify-between gap-3 px-2.5 py-1.5">
            ...
            <dd className={`truncate text-[11px] font-semibold text-slate-700 ${row.mono ? 'font-mono tabular-nums' : ''}`}>
              {row.value}
            </dd>
          </div>
        ))}
      ...
      <div className="mt-3 flex items-center gap-1 border-t border-slate-100 pt-2.5">
        <button
          ...
          className={`${iconButton} text-slate-500 hover:bg-slate-100 hover:text-slate-900`}
        >
          <Pencil size={14} />
        </button>
        <button
          ...
          className={`${iconButton} text-slate-500 hover:bg-brand-50 hover:text-brand-700`}
        >
          <Eye size={14} />
        </button>
        <button
          ...
          className={`${iconButton} text-slate-500 hover:bg-emerald-50 hover:text-emerald-700`}
        >
          <Check size={14} />
        </button>
        <button
          ...
          className={`${iconButton} text-rose-500 hover:bg-rose-50 hover:text-rose-700`}
        >
          <Trash2 size={14} />
        </button>
      </div>
...
export const TaskBucketAccordion = ({
  ...
  <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <button
      ...
      onClick={onToggle}
      aria-expanded={open}
      className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40"
    >
      <div className="flex min-w-0 items-center gap-3">
        <span className={`h-8 w-1.5 shrink-0 rounded-full ${accentClass}`} aria-hidden="true" />
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold tracking-tight text-slate-900">{label}</p>
          <p className="truncate text-[11px] text-slate-500">{hint}</p>
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-3">
        <span className="rounded-full bg-slate-100 px-2.5 py-1 font-mono text-[11px] font-semibold tabular-nums text-slate-600">
          {count}
        </span>
        <span className="hidden font-mono text-sm font-semibold tabular-nums text-slate-900 sm:inline">
          {formatINR(total)}
        </span>
    ...
    >
      <div className="min-h-0 overflow-hidden">
        <div className="border-t border-slate-100 p-4">
          {count === 0 ? (
            <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50/60 p-5 text-center text-sm text-slate-500">
              {emptyMessage}
            </p>
          ) : (
            children
