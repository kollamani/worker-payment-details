...
// Dark-glass neon palettes — cycled per member card (Neon Pink/Rose, Mint/Cyan,
...
const cardPalettes = [
  {
    ...
    action: 'border-white/10 bg-white/[0.06] text-slate-200 hover:border-rose-400/40 hover:bg-white/[0.12]',
  },
  {
    ...
    action: 'border-white/10 bg-white/[0.06] text-slate-200 hover:border-cyan-400/40 hover:bg-white/[0.12]',
  },
  {
    ...
    action: 'border-white/10 bg-white/[0.06] text-slate-200 hover:border-amber-400/40 hover:bg-white/[0.12]',
  },
  {
    ...
    action: 'border-white/10 bg-white/[0.06] text-slate-200 hover:border-purple-400/40 hover:bg-white/[0.12]',
  },
  {
    ...
    action: 'border-white/10 bg-white/[0.06] text-slate-200 hover:border-sky-400/40 hover:bg-white/[0.12]',
  },
];
...
const UsersList = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#080b11]">
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0">
        ...
        <div className="absolute bottom-0 left-0 h-80 w-80 rounded-full bg-rose-500/[0.08] blur-3xl" />
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">User Detail View</h1>
            <p className="mt-1 text-sm text-slate-400">
              Search the directory and open a member card to view their complete ledger history.
            </p>
          </div>
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-700 bg-slate-900/60 px-4 py-2 text-sm text-slate-300 backdrop-blur-md">
        ...
        <div className="relative mb-8 max-w-md">
          ...
          <input
            ...
            autoFocus
            className="w-full rounded-xl border border-slate-800 bg-slate-900/40 py-2.5 pl-10 pr-10 text-sm text-slate-100 backdrop-blur-md transition-colors placeholder:text-slate-500 focus:border-slate-600 focus:outline-none focus:ring-2 focus:ring-sky-500/20"
          />
          {search && (
            <button
              type="button"
              ...
              className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-500 hover:bg-white/10 hover:text-slate-300"
            >
              <X size={14} />
            </button>
          )}
        ...
        {loading ? (
          <p className="rounded-xl border border-slate-800 bg-slate-900/40 p-6 text-sm text-slate-400 backdrop-blur-md">Loading members...</p>
        ) : filtered.length === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30 p-6 text-sm text-slate-400">No members found.</p>
        ) : (
            ...
            {filtered.map((m, index) => {
              ...
              return (
                <Link
                  key={m._id}
                  to={`/users/${m._id}`}
                  className={`group no-underline hover:no-underline flex h-full min-h-[190px] flex-col rounded-3xl border bg-white/[0.03] p-4 backdrop-blur-md transition-all duration-300 ease-out hover:-translate-y-1 ${palette.tint} ${palette.border} ${palette.glow} ${palette.glowHover}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <p className="min-w-0 truncate font-medium tracking-tight text-white" title={m.name}>
                      {m.name}
                    </p>
                    <span className="shrink-0 rounded-full border border-white/10 bg-white/10 px-2 py-0.5 font-mono text-xs text-slate-300 backdrop-blur-sm">
                      {m.jNo || '—'}
                    </span>
                  </div>
                  <div className="mt-3 space-y-1">
                    <p className="truncate text-sm text-slate-400" title={m.villageName || 'No village'}>
                      {m.villageName || 'No village'}
                    </p>
                    <p className="truncate text-sm text-slate-400" title={String(m.createdByWorker || '').trim() || 'Unassigned'}>
                      Admin: <span className="font-medium text-slate-200">{String(m.createdByWorker || '').trim() || 'Unassigned'}</span>
                    </p>
                  </div>
